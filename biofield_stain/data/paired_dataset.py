...
class CropPairedDataset(Dataset):
    ...
    def _center_crop_pair(self, he_img, ihc_img):
        """Take the same deterministic center crop from both images."""
        w, h = he_img.size
        if w < self.crop_size or h < self.crop_size:
            raise ValueError(
                f"Image size {w}x{h} smaller than crop size {self.crop_size}"
    ...
    def _process_pair(self, he_img, ihc_img, label, filename):
        ...
        """
        # Training keeps random crops; validation/test use deterministic center crops.
        if self.augment:
