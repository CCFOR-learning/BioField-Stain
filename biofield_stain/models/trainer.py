"""
BioFieldStain: Pixel-Space UNI-Guided Virtual Staining Network.
...
from biofield_stain.models.discriminator import (
    PatchDiscriminator, MultiScaleDiscriminator,
    hinge_loss_d, hinge_loss_g, r1_gradient_penalty, feature_matching_loss,
)
from biofield_stain.models.generator import SPADEUNetGenerator
from biofield_stain.models.losses import VGGFeatureExtractor, gram_matrix, PatchNCELoss
...
# ======================================================================
# BioField Expression Module
# ======================================================================

class BioFieldExpressionHead(nn.Module):
    """Predict a marker-specific DAB expression field from generated IHC.

    The head is intentionally lightweight and auxiliary: it gives the generator
    a pathology-aware spatial target without changing the RGB output
    interface used by BioFieldStain evaluation.
    """
    ...
    def __init__(self, num_classes, hidden_dim=32):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(3 + num_classes, hidden_dim, kernel_size=3, padding=1),
    ...
    def forward(self, rgb_m11, labels):
        b, _, h, w = rgb_m11.shape
        ...
        cond = cond.to(device=rgb_m11.device, dtype=rgb_m11.float().dtype)
        x = torch.cat([rgb_m11.float(), cond], dim=1)
        return torch.sigmoid(self.net(x))


class BioFieldGatedStainAdapter(nn.Module):
    """Marker-aware residual adapter gated by a predicted DAB expression field.

    The main generator keeps responsibility for morphology and coarse stain
    synthesis. This lightweight adapter only learns a bounded local correction,
    with the BioField map acting as an expression-aware spatial gate.
    """

    def __init__(self, num_classes, hidden_dim=32, residual_scale=0.15):
        super().__init__()
        self.num_classes = num_classes
        self.residual_scale = residual_scale
        in_channels = 3 + 3 + 1 + num_classes
        groups = 8 if hidden_dim % 8 == 0 else 4
        self.net = nn.Sequential(
    ...
    def forward(self, coarse, he, biofield, labels):
        b, _, h, w = coarse.shape
        if biofield.shape[-2:] != (h, w):
            biofield = F.interpolate(
                biofield, size=(h, w), mode='bilinear', align_corners=False)

        safe_labels = labels.clamp(min=0, max=self.num_classes - 1)
        one_hot = F.one_hot(safe_labels, num_classes=self.num_classes).float()
        label_map = one_hot[:, :, None, None].expand(b, self.num_classes, h, w)
        label_map = label_map.to(device=coarse.device, dtype=coarse.float().dtype)

        x = torch.cat([coarse.float(), he.float(), biofield.float(), label_map], dim=1)
        out = self.net(x)
        residual = torch.tanh(out[:, :3])
        learned_gate = torch.sigmoid(out[:, 3:4])
        expression_gate = 0.10 + 0.90 * biofield.float().clamp(0, 1)
        refined = coarse.float() + self.residual_scale * residual * learned_gate * expression_gate
        return refined.clamp(-1, 1).to(dtype=coarse.dtype)


class BioFieldStainTrainer(pl.LightningModule):
    """PyTorch Lightning training module for BioFieldStain.
    ...
    def __init__(
        ...
        label_names=None,
        # Innovation / ablation controls
        innovation_mode='baseline',
        expr_field_weight=0.0,
        expr_pearson_weight=0.0,
        expr_iod_weight=0.0,
        expr_field_size=128,
        disentangle_weight=0.0,
        atlas_weight=0.0,
        atlas_topk=3,
        innovation_warmup_steps=2000,
        biofield_adapter=False,
        biofield_adapter_hidden=32,
        biofield_adapter_scale=0.15,
        biofield_adapter_reg_weight=0.02,
        biofield_adapter_freeze_backbone=False,
        biofield_decoder_mod=False,
        biofield_decoder_hidden=64,
        biofield_decoder_scale=0.10,
        biofield_decoder_layers=(5, 4, 3, 2),
        biofield_decoder_condition_weight=1.0,
        stain_experts=False,
        stain_expert_hidden=32,
        stain_expert_scale=0.10,
        stain_expert_layers=(4, 3, 2),
        stain_expert_adaptive_gate=False,
        stain_expert_gate_hidden=16,
        stain_expert_gate_init=0.50,
        stain_experts_freeze_backbone=False,
        adaptive_protein_loss=False,
        adaptive_protein_min_scale=0.5,
        adaptive_protein_max_scale=1.5,
        adaptive_protein_stain_norm=False,
        adaptive_atlas_loss=False,
        atlas_conf_threshold=0.25,
        atlas_conf_temperature=0.10,
        atlas_conf_min_scale=0.5,
        atlas_conf_max_scale=1.5,
    ):
        super().__init__()
        ...
        self.generator = SPADEUNetGenerator(
            ...
            stain_experts=stain_experts,
            stain_expert_hidden=stain_expert_hidden,
            stain_expert_scale=stain_expert_scale,
            stain_expert_layers=stain_expert_layers,
            stain_expert_adaptive_gate=stain_expert_adaptive_gate,
            stain_expert_gate_hidden=stain_expert_gate_hidden,
            stain_expert_gate_init=stain_expert_gate_init,
            biofield_decoder_mod=biofield_decoder_mod,
            biofield_decoder_hidden=biofield_decoder_hidden,
            biofield_decoder_scale=biofield_decoder_scale,
            biofield_decoder_layers=biofield_decoder_layers,
        )

        if stain_experts_freeze_backbone and stain_experts:
            self.generator.requires_grad_(False)
            self.generator.stain_experts.requires_grad_(True)
        ...
        biofield_requested = (
            innovation_mode in ('biofield', 'full') or
            expr_field_weight > 0 or expr_pearson_weight > 0 or expr_iod_weight > 0 or
            biofield_adapter
        )
        self.expression_head = (
            BioFieldExpressionHead(num_classes=num_classes)
            if biofield_requested else None
        )

        self.biofield_adapter = (
            BioFieldGatedStainAdapter(
                num_classes=num_classes,
                hidden_dim=biofield_adapter_hidden,
                residual_scale=biofield_adapter_scale,
            )
            if biofield_adapter else None
        )
        if biofield_adapter_freeze_backbone and self.biofield_adapter is not None:
            self.generator.requires_grad_(False)
            self.generator_ema.requires_grad_(False)
        ...
        n_expr = sum(p.numel() for p in self.expression_head.parameters()) if self.expression_head else 0
        n_adapter = sum(p.numel() for p in self.biofield_adapter.parameters()) if self.biofield_adapter else 0
        n_biofield_mod = (
            sum(p.numel() for p in self.generator.biofield_modulators.parameters())
            if getattr(self.generator, 'biofield_modulators', None) is not None else 0
        )
        n_experts = (
            sum(p.numel() for p in self.generator.stain_experts.parameters())
            if getattr(self.generator, 'stain_experts', None) is not None else 0
        )
        print(f"Generator: {n_gen:,} params")
        print(f"Discriminator: {n_disc:,} params (global) + {n_crop:,} (crop) + {n_uncond:,} (uncond)")
        if n_experts > 0:
            print(
                f"Marker-specific decoder experts: {n_experts:,} params | "
                f"layers={stain_expert_layers} scale={stain_expert_scale} "
                f"adaptive_gate={stain_expert_adaptive_gate} "
                f"freeze_backbone={stain_experts_freeze_backbone}"
            )
        if self.expression_head is not None:
            print(f"BioField expression head: {n_expr:,} params | mode={innovation_mode}")
        if n_biofield_mod > 0:
            print(
                f"BioField decoder modulation: {n_biofield_mod:,} params | "
                f"layers={biofield_decoder_layers} scale={biofield_decoder_scale} "
                f"condition_weight={biofield_decoder_condition_weight}"
            )
        if self.biofield_adapter is not None:
            print(
                f"BioField gated stain adapter: {n_adapter:,} params | "
                f"scale={biofield_adapter_scale} reg={biofield_adapter_reg_weight} "
                f"freeze_backbone={biofield_adapter_freeze_backbone}"
            )

    def configure_optimizers(self):
        gen_params = list(self.generator.parameters())
        if self.expression_head is not None:
            gen_params += list(self.expression_head.parameters())
        if self.biofield_adapter is not None:
            gen_params += list(self.biofield_adapter.parameters())
    ...
    def _get_innovation_scale(self):
        """Ramp auxiliary innovation losses without touching losses."""
        warmup = int(getattr(self.hparams, 'innovation_warmup_steps', 0))
        if warmup <= 0:
            return 1.0
        return min(1.0, float(self.global_step) / float(max(1, warmup)))
    ...
    def _load_uni_model(self):
        ...
        if self._uni_model is None:
            import timm
            local_candidates = [
                Path(__file__).resolve().parents[2] / "weights" / "uni-vit-l16" / "pytorch_model.bin",
                Path(__file__).resolve().parents[2] / "weights" / "UNI" / "pytorch_model.bin",
                Path.cwd() / "weights" / "uni-vit-l16" / "pytorch_model.bin",
            ]
            local_weights = next((p for p in local_candidates if p.is_file()), None)
            if local_weights is not None:
                ...
                self._uni_model.load_state_dict(state, strict=True)
                print(f"UNI model loaded from local weights: {local_weights}")
            else:
                self._uni_model = timm.create_model(
    ...
    def _extract_normalized_dab_field(self, images):
        """Differentiable low-resolution DAB expression map in [0, 1]."""
        dab = self.dab_extractor.extract_dab_intensity(images.float(), normalize="none")
        if dab.dim() == 3:
            dab = dab.unsqueeze(1)
        b = dab.shape[0]
        flat = dab.reshape(b, -1)
        # Per-image p99 normalization keeps the field loss scale stable across stains.
        p99 = torch.quantile(flat.detach(), 0.99, dim=1, keepdim=True).clamp(min=1e-6)
        norm = flat.clamp(max=p99) / p99
        return norm.reshape_as(dab).clamp(0.0, 1.0)

    @staticmethod
    def _map_pearson_loss(pred, target):
        return BioFieldStainTrainer._map_pearson_loss_per_sample(pred, target).mean()

    @staticmethod
    def _map_pearson_loss_per_sample(pred, target):
        ...
        corr = (pred_flat * target_flat).sum(dim=1) / denom.clamp(min=1e-6)
        return 1.0 - corr

    def compute_adaptive_protein_weights(self, target, labels=None):
        """Estimate per-sample protein-supervision weights from real DAB stats."""
        if not getattr(self.hparams, 'adaptive_protein_loss', False):
            return None

        with torch.no_grad(), torch.amp.autocast('cuda', enabled=False):
            field_size = int(self.hparams.expr_field_size)
            if target.shape[-1] != field_size:
                tgt_low = F.interpolate(
                    target.float(), size=field_size, mode='bilinear',
                    align_corners=False)
            else:
                tgt_low = target.float()

            real_field = self._extract_normalized_dab_field(tgt_low)
            flat = real_field.flatten(1)
            strength = flat.mean(dim=1) + flat.std(dim=1, unbiased=False)

            if (getattr(self.hparams, 'adaptive_protein_stain_norm', False) and
                    labels is not None):
                center = torch.zeros_like(strength)
                safe_labels = labels.detach()
                for label_idx in safe_labels.unique().tolist():
                    mask = safe_labels == int(label_idx)
                    center[mask] = strength[mask].mean()
            else:
                center = strength.mean().expand_as(strength)

            scale = strength / center.clamp(min=1e-6)
            scale = scale.clamp(
                min=float(self.hparams.adaptive_protein_min_scale),
                max=float(self.hparams.adaptive_protein_max_scale),
            )
            return scale.detach()

    @staticmethod
    def _weighted_mean(per_sample_loss, sample_weights=None):
    ...
    def make_biofield_condition(self, he, labels):
        """Predict an H&E-derived marker expression prior for decoder modulation."""
        if not getattr(self.hparams, 'biofield_decoder_mod', False):
            return None
        if self.expression_head is None:
            return None
        with torch.amp.autocast('cuda', enabled=False):
            field_size = int(self.hparams.expr_field_size)
            he_low = F.interpolate(
                he.float(), size=field_size, mode='bilinear', align_corners=False
            ) if he.shape[-1] != field_size else he.float()
            return self.expression_head(he_low, labels)

    def compute_biofield_condition_losses(self, condition_field, target, sample_weights=None):
        """Supervise the H&E-derived BioField condition against real IHC DAB."""
        if condition_field is None:
            zero = torch.tensor(0.0, device=target.device, requires_grad=True)
            return zero, zero, zero
        with torch.amp.autocast('cuda', enabled=False):
            field = condition_field.float()
            field_size = field.shape[-1]
            tgt_low = F.interpolate(
                target.float(), size=field_size, mode='bilinear', align_corners=False
            ) if target.shape[-1] != field_size else target.float()
            real_field = self._extract_normalized_dab_field(tgt_low).detach()

            field_per_sample = (field - real_field).abs().flatten(1).mean(dim=1)
            pearson_per_sample = self._map_pearson_loss_per_sample(field, real_field)
            iod_per_sample = (
                field.flatten(1).mean(dim=1) - real_field.flatten(1).mean(dim=1)
            ).abs()
            return (
                self._weighted_mean(field_per_sample, sample_weights),
                self._weighted_mean(pearson_per_sample, sample_weights),
                self._weighted_mean(iod_per_sample, sample_weights),
            )

    def compute_biofield_losses(self, generated, target, labels, sample_weights=None):
        """BioField losses: spatial DAB field, Pearson alignment, and IOD alignment."""
        if self.expression_head is None:
            zero = torch.tensor(0.0, device=generated.device, requires_grad=True)
            return zero, zero, zero

        with torch.amp.autocast('cuda', enabled=False):
            field_size = int(self.hparams.expr_field_size)
            if generated.shape[-1] != field_size:
                gen_low = F.interpolate(generated.float(), size=field_size, mode='bilinear', align_corners=False)
                tgt_low = F.interpolate(target.float(), size=field_size, mode='bilinear', align_corners=False)
            else:
                gen_low = generated.float()
                tgt_low = target.float()

            pred_field = self.expression_head(gen_low, labels)
            gen_dab_field = self._extract_normalized_dab_field(gen_low)
            real_field = self._extract_normalized_dab_field(tgt_low).detach()

            field_per_sample = (
                0.5 * (pred_field - real_field).abs().flatten(1).mean(dim=1) +
                0.5 * (gen_dab_field - real_field).abs().flatten(1).mean(dim=1)
            )
            pearson_per_sample = (
                0.5 * self._map_pearson_loss_per_sample(pred_field, real_field) +
                0.5 * self._map_pearson_loss_per_sample(gen_dab_field, real_field)
            )
            real_mean = real_field.flatten(1).mean(dim=1)
            pred_mean = pred_field.flatten(1).mean(dim=1)
            gen_mean = gen_dab_field.flatten(1).mean(dim=1)
            iod_per_sample = (
                0.5 * (pred_mean - real_mean).abs() +
                0.5 * (gen_mean - real_mean).abs()
            )

            loss_field = self._weighted_mean(field_per_sample, sample_weights)
            loss_pearson = self._weighted_mean(pearson_per_sample, sample_weights)
            loss_iod = self._weighted_mean(iod_per_sample, sample_weights)
            return loss_field, loss_pearson, loss_iod

    def apply_biofield_adapter(self, generated, he, labels):
        """Apply the optional BioField-gated residual adapter."""
        if self.biofield_adapter is None:
            return generated, torch.tensor(0.0, device=generated.device)
        if self.expression_head is None:
            raise RuntimeError("BioField adapter requires expression_head to be enabled.")
        with torch.amp.autocast('cuda', enabled=False):
            field = self.expression_head(generated.float(), labels)
            adapted = self.biofield_adapter(generated.float(), he.float(), field, labels)
            delta = F.l1_loss(adapted.float(), generated.float().detach())
        return adapted.to(dtype=generated.dtype), delta

    def compute_atlas_prototype_loss(self, generated, target, uni_features, labels):
        """In-batch pathology atlas prototype loss.

        Each sample retrieves same-stain neighbours in the current mini-batch by
        UNI morphology-token similarity, then uses their real DAB fields as a
        local pathology prototype. This is a lightweight training-time version of
        a larger retrieval atlas and gives a clean ablation knob.
        """
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            gen_field = self._extract_normalized_dab_field(gen_low)
            real_field = self._extract_normalized_dab_field(tgt_low).detach()

            tokens = uni_features.float()
            if tokens.dim() == 3:
                morph = tokens.mean(dim=1)
            else:
                morph = tokens.reshape(tokens.shape[0], -1)
            ...
            valid = same_label & (~eye)
            sim = sim.masked_fill(~valid, -1e4)
            k = min(int(self.hparams.atlas_topk), max(1, labels.shape[0] - 1))
            top_vals, top_idx = sim.topk(k=k, dim=1)
            ...
            for i in range(labels.shape[0]):
                proto = (real_field[top_idx[i]] * weights[i].view(-1, 1, 1, 1)).sum(dim=0)
                prototypes.append(proto)
            proto_field = torch.stack(prototypes, dim=0).detach()

            map_per_sample = (
                gen_field[valid_rows] - proto_field[valid_rows]
            ).abs().flatten(1).mean(dim=1)
            corr_per_sample = self._map_pearson_loss_per_sample(
                gen_field[valid_rows], proto_field[valid_rows])

            atlas_weights = None
            if getattr(self.hparams, 'adaptive_atlas_loss', False):
                conf_score = (top_vals * weights).sum(dim=1)[valid_rows]
                conf = torch.sigmoid(
                    (conf_score - float(self.hparams.atlas_conf_threshold)) /
                    max(float(self.hparams.atlas_conf_temperature), 1e-6)
                )
                atlas_weights = (
                    float(self.hparams.atlas_conf_min_scale) +
                    (float(self.hparams.atlas_conf_max_scale) -
                     float(self.hparams.atlas_conf_min_scale)) * conf
                ).detach()
                self.log('train/atlas_conf_mean', conf.mean(), prog_bar=False)
                self.log('train/atlas_scale_mean', atlas_weights.mean(),
                         prog_bar=False)

            loss_map = self._weighted_mean(map_per_sample, atlas_weights)
            loss_corr = self._weighted_mean(corr_per_sample, atlas_weights)
            return loss_map + 0.25 * loss_corr

    def compute_dab_intensity_loss(self, generated, target, sample_weights=None):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            per_sample = (gen_scores - tgt_scores).abs()
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Generator step
        # ----------------------------------------------------------------
        biofield_condition = self.make_biofield_condition(he, labels_dropped)
        generated = self.generator(
            he, uni_dropped, labels_dropped, biofield_condition=biofield_condition)
        generated, loss_adapter_delta = self.apply_biofield_adapter(
            generated, he, labels)
        ...
        if self.biofield_adapter is not None and self.hparams.biofield_adapter_reg_weight > 0:
            loss_g = loss_g + self.hparams.biofield_adapter_reg_weight * loss_adapter_delta
            self.log('train/biofield_adapter_delta', loss_adapter_delta, prog_bar=False)
        ...
        # Protein-aware adaptive scaling for DAB/BioField supervision.
        protein_weights = self.compute_adaptive_protein_weights(her2, labels)
        if protein_weights is not None:
            self.log('train/adaptive_protein_scale_mean',
                     protein_weights.mean(), prog_bar=False)
            self.log('train/adaptive_protein_scale_min',
                     protein_weights.min(), prog_bar=False)
            self.log('train/adaptive_protein_scale_max',
                     protein_weights.max(), prog_bar=False)

        # DAB losses (use original labels, not dropped)
        if self.hparams.dab_intensity_weight > 0:
            loss_dab = self.compute_dab_intensity_loss(
                generated, her2, sample_weights=protein_weights)
        ...
        # BioField-IHC: explicit marker expression field supervision.
        innovation_scale = self._get_innovation_scale()
        if self.expression_head is not None and (
            self.hparams.expr_field_weight > 0 or
            self.hparams.expr_pearson_weight > 0 or
            self.hparams.expr_iod_weight > 0
        ):
            loss_expr_field, loss_expr_pearson, loss_expr_iod = self.compute_biofield_losses(
                generated, her2, labels, sample_weights=protein_weights)
            loss_g = loss_g + innovation_scale * self.hparams.expr_field_weight * loss_expr_field
            loss_g = loss_g + innovation_scale * self.hparams.expr_pearson_weight * loss_expr_pearson
            loss_g = loss_g + innovation_scale * self.hparams.expr_iod_weight * loss_expr_iod
            self.log('train/innovation_scale', innovation_scale, prog_bar=False)
            self.log('train/biofield_l1', loss_expr_field, prog_bar=False)
            self.log('train/biofield_pearson_loss', loss_expr_pearson, prog_bar=False)
            self.log('train/biofield_iod', loss_expr_iod, prog_bar=False)

        if biofield_condition is not None and self.hparams.biofield_decoder_condition_weight > 0:
            # Supervise the condition prior with true marker labels, not CFG-dropped null labels.
            condition_for_loss = biofield_condition
            if torch.any(labels_dropped != labels):
                condition_for_loss = self.make_biofield_condition(he, labels)
            cond_l1, cond_pearson, cond_iod = self.compute_biofield_condition_losses(
                condition_for_loss, her2, sample_weights=protein_weights)
            cond_weight = innovation_scale * self.hparams.biofield_decoder_condition_weight
            loss_g = loss_g + cond_weight * self.hparams.expr_field_weight * cond_l1
            loss_g = loss_g + cond_weight * self.hparams.expr_pearson_weight * cond_pearson
            loss_g = loss_g + cond_weight * self.hparams.expr_iod_weight * cond_iod
            self.log('train/biofield_cond_l1', cond_l1, prog_bar=False)
            self.log('train/biofield_cond_pearson_loss', cond_pearson, prog_bar=False)
            self.log('train/biofield_cond_iod', cond_iod, prog_bar=False)
        ...
        # Morphology-expression disentanglement: keep morphology in an H&E-like
        # structural stream while BioField carries marker expression.
        if self.hparams.disentangle_weight > 0:
            loss_disentangle = self.compute_he_edge_loss(generated, he)
            loss_g = loss_g + innovation_scale * self.hparams.disentangle_weight * loss_disentangle
            self.log('train/disentangle_morph', loss_disentangle, prog_bar=False)

        # Pathology atlas prototype: same-stain in-batch retrieval regularization.
        if self.hparams.atlas_weight > 0:
            loss_atlas = self.compute_atlas_prototype_loss(generated, her2, uni, labels)
            loss_g = loss_g + innovation_scale * self.hparams.atlas_weight * loss_atlas
            self.log('train/atlas_proto', loss_atlas, prog_bar=False)
        ...
        if self.expression_head is not None:
            torch.nn.utils.clip_grad_norm_(self.expression_head.parameters(), 1.0)
        if self.biofield_adapter is not None:
            torch.nn.utils.clip_grad_norm_(self.biofield_adapter.parameters(), 1.0)
        opt_g.step()
        ...
        if self.global_step >= self.hparams.adversarial_start_step and any_adv:
            with torch.no_grad():
                fake_condition = self.make_biofield_condition(he, labels_dropped)
                fake_detached = self.generator(
                    he, uni_dropped, labels_dropped, biofield_condition=fake_condition)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Use EMA generator
        with torch.no_grad():
            biofield_condition = self.make_biofield_condition(he, labels)
            generated = self.generator_ema(
                he, uni, labels, biofield_condition=biofield_condition)
            generated, _ = self.apply_biofield_adapter(generated, he, labels)
        ...
        if self.expression_head is not None:
            val_expr_field, val_expr_pearson, val_expr_iod = self.compute_biofield_losses(
                generated, her2, labels)
            self.log('val/biofield_l1', val_expr_field, prog_bar=False, sync_dist=True)
            self.log('val/biofield_pearson_loss', val_expr_pearson, prog_bar=False, sync_dist=True)
            self.log('val/biofield_iod', val_expr_iod, prog_bar=False, sync_dist=True)

        if self.hparams.atlas_weight > 0:
            val_atlas = self.compute_atlas_prototype_loss(generated, her2, uni, labels)
            self.log('val/atlas_proto', val_atlas, prog_bar=False, sync_dist=True)
    ...
    def generate(self, he_images, uni_features, labels,
        ...
        if guidance_scale <= 1.0:
            biofield_condition = self.make_biofield_condition(he_images, labels)
            output = gen(
                he_images, uni_features, labels, biofield_condition=biofield_condition)
        ...
        biofield_condition = self.make_biofield_condition(he_images, labels)
        null_biofield_condition = self.make_biofield_condition(he_images, null_labels)
        output_cond = gen(
            he_images, uni_features, labels, biofield_condition=biofield_condition)
        output_uncond = gen(
            he_images, uni_features, null_labels, biofield_condition=null_biofield_condition)
