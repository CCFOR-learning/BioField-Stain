#!/usr/bin/env python3
"""
Evaluate BioFieldStain on MIST dataset at native 1024x1024 resolution.
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate BioFieldStain on MIST (1024)')
    ...
    print("=" * 70)
    print(f"EVALUATION: BioField-Stain on MIST — 1024x1024")
    print(f"  Stains: {args.stains}")
    print("=" * 70)
    print(f"Checkpoint: {args.checkpoint}")

    # Load model
    model = BioFieldStainTrainer.load_from_checkpoint(args.checkpoint, strict=False)
    model = model.cuda().eval()
